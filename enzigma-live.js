    ...
    if (hasInvalidPlus) {
        ...
        $error.text("Phone must be 7-15 digits.").show();
    } else {
        $error.hide();
    }
    });
    ...
    // Add URL validation for textarea
        ...
        // Select validation
        $('#00NF800000AaSlq').on('change', function() {
        ...
        // Check select field first
        var selectField = document.getElementById('00NF800000AaSlq');
        var hasSelection = false;
        for (var i = 0; i < selectField.options.length; i++) {
            if (selectField.options[i].selected && selectField.options[i].value !== '') {
                hasSelection = true;
                break;
            }
        }
        ...
        if (phone.length === 0) {
            $('#phone-error-message').text('Phone number is required').show();
            hasErrors = true;
        } else if (hasInvalidPlus) {
            $('#phone-error-message').text("'+' symbol is only allowed at the beginning.").show();
            hasErrors = true;
        } else if (isDigitCountInvalid) {
            $('#phone-error-message').text('Phone must be 7-15 digits..').show();
            hasErrors = true;
        } else {
        ...
        // Select validation - hard check
        var selectElement = document.getElementById('00NF800000AaSlq');
        var hasValidSelection = false;
        for (var i = 0; i < selectElement.selectedOptions.length; i++) {
            if (selectElement.selectedOptions[i].value !== '') {
    ...
    //  document.querySelector(".blog-contact-form-container").addEventListener("submit", function (e) {
    ...
    //     // Email validation
    //     var isEmailValid = allowedDomains.some(function (domain) {
    ...
    //     var select = document.getElementById("00NF800000AaSlq");
    //     var selectedOptions = Array.from(select.selectedOptions).filter(function (option) {
    //       return option.value !== "";
    //     });

    //     if (selectedOptions.length === 0) {
    //       e.preventDefault();
    //       document.getElementById("select-error").style.display = "inline";
    //       select.focus();
    //     } else {
